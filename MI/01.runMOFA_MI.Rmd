...
pagoda_hvgs <- pagoda_hvgs %>%
  enframe("name","gene") %>%
  unnest()

# Filtering out black listed genes
...
mrkr_genes <- mrkr_genes %>% #dplyr::filter(!name %in% exclude_ct) %>%
  ...
  dplyr::mutate(gene_type = "marker_gene")

# Here are genes that aren't cell type markers
ok_genes <- pagoda_hvgs %>%
  left_join(mrkr_genes, by = "gene") %>%
...
pagoda_hvgs <- bind_rows(ok_genes, 
                         blacklist_genes) %>%
  group_by(name) %>%
    nest() %>%
    dplyr::mutate(data = map(data, ~.x[[1]])) %>%
